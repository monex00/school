...
public: SocketUDP();
		...
		bool sendToRaw(Address, void*, int);
		void* recvFromRaw(Address&, int*);

		bool sendToFile(Address, char*);
		bool recvFromFile(Address&, char*, int*);
...
bool SocketUDP::sendToString(Address destAddress, char* msg) {
	return SocketUDP::sendToRaw(destAddress, (void*) msg, strlen(msg) + 1);
}
...
char* SocketUDP::recvFromString(Address& sendAddress) {
	int len;
	char* msg = (char*) SocketUDP::recvFromRaw(sendAddress, &len);
...
bool SocketUDP::sendToRaw(Address destAddress, void* msg, int len) {
	bool retVal = false;
...
void* SocketUDP::recvFromRaw(Address& sendAddress, int* len) {
	...
	// addrLen -> lunghezza della struttura passata come parametro (in questo caso
...
bool SocketUDP::sendToFile(Address destAddress, char* path) {
	printf("[1.1]"); fflush(stdout);
	FILE* file = fopen(path, "r");
	if(file == NULL) printf("NULL"); fflush(stdout);
	printf("[1.2]"); fflush(stdout);
	// TODO: controllare che se path punta a null il programma vada in errore nella sendToRaw 
	bool ret = SocketUDP::sendToRaw(destAddress, (void*)file, getFileLen(file));
	printf("[1.3]"); fflush(stdout);
	fclose(file);
	printf("[1.4]"); fflush(stdout);
	return ret;
}

bool SocketUDP::recvFromFile(Address& sendAddress, char* path, int* len) {
	FILE* inFile = (FILE*) SocketUDP::recvFromRaw(sendAddress, len);
	if(inFile = NULL) return false;
	// TODO: controllare se si vuole che non sovrascriva
	FILE* outFile = fopen(path, "w");
	for(int i = 0; i <= *len; fputc(fgetc(inFile), outFile), i++);
	fclose(outFile);
	return true;
}

long SocketUDP::getFileLen(FILE* file) {
